...
import {
  checkPastHandles,
  convertToBskyPostUrl,
  getBskyPostUrl
} from './libs/urlHandler'
  ...
  if (urls.length > 0) {
    ...
    for (let index = 0; index < urls.length; index++) {
      ...
      const newUrl = await Promise.race([
        ...
        urlMappings.find(({ url }) => urls[index] == url)?.expanded_url ??
          resolveShorURL(urls[index])
      ]).catch((err) => {
      ...
      if (
        checkPastHandles(twitterHandles, newUrl) &&
    ...
    if (newUrls.length > 0) {
      ...
      newText = URI.withinString(tweetFullText, (url, start, end, source) => {
        ...
        if (
          (checkPastHandles(twitterHandles, newUrls[j]) &&
...
function saveCache(sortedTweets) {
  ...
  FS.writeFileSync(
    TWEETS_MAPPING_FILE_NAME,
    JSON.stringify(sortedTweets, null, 4)
  )
}

async function fetchEmbedUrlCard(
  url: string,
          ...
          oembetter.fetch(
            url,
            {
              headers: {
                'User-Agent': USER_AGENT,
                Accept:
                  'text/html,application/xhtml+xml,application/xml;q=0.9,image/jpeg,image/webp,image/apng,*/*;q=0.8',
    ...
    if (
      ...
      if (!resp.ok) {
        if (resp.status == 401 && url.startsWith('http:')) {
          console.warn(
            `HTTP error: ${resp.status} ${resp.statusText} (try with https)`
          )
          return await fetchEmbedUrlCard(
      ...
      const descriptionTag = $('meta[property="og:description"]').attr(
        'content'
      )
      if (descriptionTag) {
...
async function removeMetadataTags(
  ...
  try {
    ...
    return new ArrayBuffer(strippedBuffer.buffer.byteLength)
  } catch (error: unknown) {
...
async function recompressImageIfNeeded(
...
async function main() {
  const argv = yargs(hideBin(process.argv))
    .option('simulate', {
      type: 'boolean',
      description:
        'Simulate the import without making any changes (defaults to false)',
    ...
    .option('ignore-video-errors', {
      type: 'boolean',
      description:
        'Continue processing tweets when the video service returns a job without an ID (usually because the video is too long)',
      default: process.env.IGNORE_VIDEO_ERRORS === '1'
    })
    .option('video-upload-retries', {
      type: 'number',
      description:
        'Number of times to retry video uploads when error JOB_STATE_FAILED encountered',
      default: process.env.VIDEO_UPLOAD_RETRIES
        ? parseInt(process.env.VIDEO_UPLOAD_RETRIES)
        : 1
    })
  ...
  if (tweets != null && tweets.length > 0) {
    ...
    await rateLimitedAgent.login({
      identifier: argv.blueskyUsername,
      password: argv.blueskyPassword
    })
    ...
    try {
      for (let index = 0; index < sortedTweets.length; index++) {
        ...
        if (minDate != undefined && tweetDate < minDate) continue
        if (maxDate != undefined && tweetDate > maxDate) break
        ...
        if (tweet.in_reply_to_screen_name) {
          if (
            argv.twitterHandles.some(
              (handle) => tweet.in_reply_to_screen_name == handle
            )
          ) {
        ...
        if (tweet.extended_entities?.media) {
          for (
            ...
            if (media?.type === 'photo') {
              ...
              let mediaFilename = `${argv.archiveFolder}${path.sep}data${
                path.sep
              }tweets_media${path.sep}${tweet.id}-${media?.media_url.substring(
                i + 1
              )}`
              ...
              if (localMediaFileNotFound) {
                ...
                const files = FS.readdirSync(path.dirname(wildcardPath)).filter(
                  (file) => file.startsWith(`${tweet.id}-`)
                )

                if (files.length > 0) {
                  mediaFilename = path.join(
                    path.dirname(wildcardPath),
                    files[0]
                  )
                  localMediaFileNotFound = false
                }
              ...
              if (localMediaFileNotFound) {
                console.warn(
                  `Local media file not found into archive. Local path: ${mediaFilename}`
                )
                continue
              }
              ...
              if (!argv.simulate) {
                const blobRecord = await rateLimitedAgent.uploadBlob(
                  imageBuffer,
                  {
                    encoding: mimeType
                  }
            ...
            if (media?.type === 'video' || media?.type === 'animated_gif') {
              ...
              for (let v = 0; v < media?.video_info?.variants?.length; v++) {
                videoFileName = media.video_info.variants[v].url
                  .split('/')
                  .pop()!
              ...
              if (localVideoFileNotFound) {
                console.warn(
                  `Local video file not found into archive. Local path: ${videoFilePath}`
                )
                continue
              }
              ...
              const videoDuration =
                parseFloat(media.video_info?.duration_millis || '0') / 1000
              const MAX_VIDEO_DURATION = 60 // Bluesky's limit in seconds

              if (videoDuration > MAX_VIDEO_DURATION) {
                console.warn(
                  ` Video duration (${videoDuration.toFixed(
                    2
                  )} sec) exceeds Bluesky's ${MAX_VIDEO_DURATION}-second limit. Video will be skipped.`
                )
                continue
              }
              ...
              if (!argv.simulate) {
                const { data: serviceAuth } =
                  await rateLimitedAgent.getServiceAuth({
                ...
                uploadUrl.searchParams.append(
                  'did',
                  rateLimitedAgent.session!.did
                )
                uploadUrl.searchParams.append(
                  'name',
                  videoFilePath.split('/').pop()! + '1'
                )
                ...
                if (jobStatus.error) {
                  console.warn(
                    ` Video job status: '${jobStatus.error}'. Video will not be posted.`
                  )
                ...
                if (!jobStatus.jobId) {
                  console.warn(
                    ' Missing jobId in response. Video will not be posted.'
                  )
                  if (!argv.ignoreVideoErrors) {
        ...
        if (!argv.simulate) {
          postText = await cleanTweetText(
            argv.twitterHandles,
            argv.blueskyUsername,
            tweet.full_text,
            tweet.entities?.urls,
            embeddedUrl,
            sortedTweets
  ...
  if (argv.simulate) {
    ...
    const minutes =
      Math.round((importedTweet * argv.apiDelay) / 1000 / 60) + 1 / 0.1
